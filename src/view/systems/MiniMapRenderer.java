...
public class MiniMapRenderer {
    ...
    public MiniMapRenderer(Camera camera, OverlayRenderer overlayRenderer) {
        this.camera = camera;
        this.overlayRenderer = overlayRenderer;
    }

    public void renderMiniMap(World world, GameMap gameMap, Graphics2D g2d, boolean showMiniMap, boolean showEntitiesOnMinimap) {
        ...
        float worldW = gameMap.getCols() * config.TILE_SIZE;
        float worldH = gameMap.getRows() * config.TILE_SIZE;
        ...
        int x0 = config.MINIMAP_MARGIN;
        int y0 = config.MINIMAP_MARGIN;
    ...
    public void rebuildMiniMapCache(GameMap gameMap) {
        ...
        int mapW = config.MINIMAP_SIZE;
        int mapH = Math.max(config.MINIMAP_MIN_HEIGHT, Math.round(config.MINIMAP_SIZE * (worldH / worldW)));
        ...
        for (int tx = 0; tx < gameMap.getCols(); tx++) {
            for (int ty = 0; ty < gameMap.getRows(); ty++) {
                float wx = tx * config.TILE_SIZE + config.TILE_SIZE / 2.0f;
                float wy = ty * config.TILE_SIZE + config.TILE_SIZE / 2.0f;

                if (gameMap.isBridgeTile(wx, wy)) {
                    g.setColor(new Color(140, 92, 50));
